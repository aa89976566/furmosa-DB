...
- 必須依序、個別向 Claude 提出三個問題，每題收到具體回覆後才能提出下一題：
  1. 這個任務最小且安全的修改範圍是什麼？哪些檔案或功能不得改動？
  2. 哪些回歸、邊界案例、安全或資料風險最容易被忽略？
  3. 應使用哪些驗收條件、測試與回復方式，才能證明修改可安全交付？
- 三題完成後，必須整理「共識、分歧、採用方案與不採用理由」，再生成 Cursor prompt。
- 收到 Claude 回覆後，必須先吸收修正，再送給 Cursor；若 Claude 指出需求仍不明確或有重大風險，暫停並向使用者說明。
- Claude 回覆必須來自實際可驗證的 Claude 對話或整合；若目前環境無法連接 Claude，必須停止並回報，不得自行模擬 Claude 回覆。
- 不得先送 Cursor 再補問 Claude，也不得把多個功能合併成一個大型 prompt。
- Cursor prompt 必須固定包含：目標、現況證據、允許修改範圍、禁止修改範圍、逐步流程、驗收條件、測試指令、停止條件與輸出格式。
- 送出 Cursor prompt 前，先記錄工作樹狀態與允許修改的檔案清單；Cursor 執行後逐一核對 `git status`、`git diff --stat` 與完整 diff。
- Cursor 若修改 prompt 範圍外的檔案、刪除既有功能、降低測試或自行擴張需求，立即停止，不得繼續部署；先列出偏離內容並要求修正。
- 不得允許 Cursor 自行改寫原始 prompt。需求有變動時，必須回到 Claude 三題審查流程，產生一份新的版本化 prompt。
