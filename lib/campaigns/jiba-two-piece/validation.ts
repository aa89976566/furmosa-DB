...
const FIELD_COMMAND_BLOCKLIST =
  /^(?:我要參加|要|可以|好|來吧|我想參加|怎麼參加|算我一個|敢|敢，來吧|這個我可以！|\+1|yes|這次先不要|不要|先不要|我再想一下|開箱|開箱文|開箱任務|ugc|試吃開箱|開箱合作|合作開箱|先看看規則|活動中心|嗷嗚計劃|換罐計劃|回家|取消|重來|查看目前資料|選門市\s*\d+|重選門市|手動輸入門市|我同意|同意|不同意|我想再看一次|略過|資料正確，送出|修改收件資料|修改門市|先不要送出|現在付款|我已轉帳|稍後再說|我要轉帳|轉帳資訊|等等再付|先不用|選雞霸兩片|選青蛙凍乾|選貓草雞肉乾|好，開始填資料|我了解用途，開始填資料|我了解，開始填資料|接著上次開箱|重新開始開箱)$/i;
