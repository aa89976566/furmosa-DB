...
describe('isRegisterNavLeaveText', () => {
  ...
  it('換罐選單捷徑應離開開戶（含介紹／換罐閘道）', () => {
    ...
    assert.equal(isRegisterNavLeaveText('輸入空罐序號'), true);
    assert.equal(isRegisterNavLeaveText('我要換罐'), true);
    assert.equal(isRegisterNavLeaveText('了解更多'), true);
  });
