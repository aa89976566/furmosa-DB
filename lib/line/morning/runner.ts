...
import {
  ...
  renderJokeMessage,
  renderNewsMessage,
} from '@/lib/line/morning/renderer';
import {
  frequencyMatchesDay,
  isSlotDue,
...
export type MorningPlanResult = {
  ...
  skipReason?: string;
  renderedText?: string;
  contentKind?: 'joke' | 'news' | 'animal_fact';
  contentId?: string;
...
export async function planOneRecipient(opts: {
  ...
  // 探測可用性 → domain decision（零擴張 consent；ALTERNATE 不走 FACT）
  ...
  if (decision.outcome === 'SKIP') {
    const skipReason =
      decision.reason === 'no_safe_news'
        ? MORNING_SKIP_REASONS.NO_SAFE_NEWS
        : decision.reason === 'not_opted_in'
          ? MORNING_SKIP_REASONS.NOT_OPTED_IN
          : MORNING_SKIP_REASONS.NO_CONTENT;
    const renderedText =
  ...
  if (decision.contentType === 'NEWS' && news) {
    const newsRow = await prisma.lineMorningNewsItem.upsert({
      ...
      create: {
        ...
        provider: news.sourceId,
        itemId: news.contentHash.slice(0, 16),
        attribution: news.sourceName,
      },
      update: {
        status: news.status,
        riskLevel: news.riskLevel,
    ...
    const recorded = await recordMorningDelivery({
      ...
      taipeiDate,
      status: 'DRY_RUN',
      contentKind: contentTypeToDeliveryKind('NEWS'),
  ...
  if (decision.contentType === 'ANIMAL_FACT' && animalFact) {
    const rendered = renderAnimalFactMessage({
      factSummary: animalFact.factSummary,
      barkLine: animalFact.barkLine,
      attribution: animalFact.attribution,
      canonicalUrl: animalFact.canonicalUrl,
      sourcePublishedAt: animalFact.sourcePublishedAt,
    });
    if (rendered.issues?.some((i) => i.startsWith('fact_news_impersonation') || i === 'missing_animal_fact_disclosure')) {
      const recorded = await recordMorningDelivery({
        lineUserId: opts.recipient.lineUserId,
        taipeiDate,
        status: 'SKIPPED',
        ...
        slotMinute,
        renderedText: 'animal_fact_render_contract_failed',
      });
      return {
        lineUserId: opts.recipient.lineUserId,
        taipeiDate,
    ...
    const recorded = await recordMorningDelivery({
      ...
      taipeiDate,
      status: 'DRY_RUN',
      contentKind: contentTypeToDeliveryKind('ANIMAL_FACT'),
      animalFactId: animalFact.id,
      slotMinute,
      renderedText: rendered.text,
    });
    if (recorded.created && opts.markUsed) {
      await markAnimalFactUsed(animalFact.id, now);
    }
    return {
      lineUserId: opts.recipient.lineUserId,
      taipeiDate,
  ...
  const recorded = await recordMorningDelivery({
    ...
    taipeiDate,
    status: 'DRY_RUN',
    contentKind: contentTypeToDeliveryKind('HUMOR'),
    contentId: joke.id,
    slotMinute,
    renderedText: rendered.text,
  });
