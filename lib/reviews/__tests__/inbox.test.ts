...
import {
  PENDING_RESTOCK_REVIEW_STATUSES,
  reviewInboxTotal,
  reviewKindLabel,
...
describe('review inbox labels', () => {
  ...
  it('includes every restock state that still needs HQ review', () => {
    assert.deepEqual(PENDING_RESTOCK_REVIEW_STATUSES, ['submitted', 'under_review']);
  });
});
