import { searchStores, type Directory, type Store } from './store-search';
...
export function createSearchService(deps: {
  ...
  return async function search(authenticated: boolean, query: string, temperature: string) {
    ...
    if (query.trim().length < 2) return { status: 200, body: { stores: [] } };
