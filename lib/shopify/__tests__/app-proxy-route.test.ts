...
test('invalid signature and disabled production deployment fail closed before provider access', async () => {
...
test('production requires its explicit flag and the live read-only directory', async () => {
  const enabled = harness({
    VERCEL_ENV: 'production',
    SHOPIFY_APP_PROXY_ENABLED: 'true',
    PICKUP_DIRECTORY_SOURCE: 'live-readonly',
  });
  assert.equal((await enabled.get(valid())).status, 200);
  assert.equal(enabled.calls(), 1);

  const stage = harness({
    VERCEL_ENV: 'production',
    SHOPIFY_APP_PROXY_ENABLED: 'true',
    PICKUP_DIRECTORY_SOURCE: 'stage',
  });
  assert.equal((await stage.get(valid())).status, 401);
