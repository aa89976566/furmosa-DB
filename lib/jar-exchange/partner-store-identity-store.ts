...
export async function listActiveIdentityDecisions(
  scope: PartnerStoreIdentityScope = identityDecisionScope(),
): Promise<PartnerStoreHumanDecision[]> {
  return activeHumanDecisions(await listIdentityDecisions(scope));
}
...
export async function createIdentityDecision(input: {
  ...
  const missing = missingHumanDecisionFields({
    merchantId,
    verdict: input.verdict,
    decidedByUserId: input.decidedByUserId,
    decidedByAccount: input.decidedByAccount,
  ...
  if (
    legacySlug &&
    active.some((row) => row.legacySlug?.toLowerCase() === legacySlug)
  ) {
    return { ok: false, error: `${legacySlug} 已有未撤銷的確認` };
  }
