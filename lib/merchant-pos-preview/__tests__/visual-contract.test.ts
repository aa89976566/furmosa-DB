...
describe('merchant POS preview visual contract (source/static; not a live viewport)', () => {
  ...
  it('defines preview-local black primary, secondary, danger, and disabled styles', () => {
    ...
    assert.match(css, /\.actionPrimary\s*\{[\s\S]*background:\s*#303030/i);
    assert.match(css, /\.actionPrimary\s*\{[\s\S]*color:\s*#ffffff/i);
    assert.match(css, /\.actionPrimary:hover[\s\S]*background:\s*#1f1f1f/i);
    assert.match(css, /\.actionPrimary:active[\s\S]*background:\s*#141414/i);
    assert.match(css, /\.actionSecondary\s*\{[\s\S]*background:\s*#ffffff/i);
    assert.match(css, /\.actionSecondary\s*\{[\s\S]*border:\s*1px solid #8a8a8a/i);
    assert.match(css, /\.actionSecondary\s*\{[\s\S]*color:\s*#191919/i);
    assert.match(css, /\.actionDanger\s*\{[\s\S]*background:\s*#c70a24/i);
    ...
    assert.match(css, /\.action:disabled[\s\S]*background:\s*#f1f1f1/i);
    assert.match(css, /\.action:disabled[\s\S]*color:\s*#8a8a8a/i);
    assert.match(css, /\.action:disabled[\s\S]*opacity:\s*1/);
    assert.match(css, /\.action:focus-visible\s*\{[\s\S]*outline:\s*2px solid #005bd3/i);
    ...
    assert.match(css, /\.productHeading\s*\{[\s\S]*display:\s*grid/i);
    assert.doesNotMatch(css, /@media\s*\(min-width:\s*1280px\)[\s\S]*\.checkoutSplit\s+\.productGrid[\s\S]*repeat\(3/i);
    ...
    assert.match(css, /\.disclosure\[open\] \.disclosureSummary::after\s*\{[\s\S]*rotate\(180deg\)/i);
