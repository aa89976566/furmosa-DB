// Explicit, bounded deployment step: applies only the approved HQ passkey
// migration. Never runs unrelated migrations or rewrites existing account data.
...
const name = '20260918162000_hq_passkeys';
...
try {
  ...
  console.error('Unable to inspect the approved HQ passkey migration state.');
...
if (finished.length > 1 || (finished.length === 1 && finished[0].checksum !== checksum)) {
  console.error(`HQ passkey migration checksum/history differs; stop for review: ${name}`);
  process.exit(1);
}
if (finished.length === 0 && unfinished.length > 0) {
  console.error(`HQ passkey migration has an unfinished production attempt; stop for review: ${name}`);
  process.exit(1);
}
...
BEGIN
 IF NOT EXISTS (SELECT 1 FROM _prisma_migrations WHERE migration_name='${name}' AND finished_at IS NOT NULL AND checksum='${checksum}') THEN
  RAISE EXCEPTION 'HQ passkey migration state changed; rerun release: ${name}';
 END IF;
END $guard$;`
  : `DO $guard$
BEGIN
...
SELECT pg_advisory_xact_lock(hashtextextended('hq:passkey-deployment:v1',0));
${applySql}
COMMIT;`;
const dir = mkdtempSync(join(tmpdir(), 'hq-passkeys-'));
...
try {
  ...
  if (result.status !== 0) {
    console.error('HQ passkey migration failed; database transaction rolled back.');
    console.error(sanitize(result.stderr));
    process.exitCode = 1;
  } else {
    console.log('Approved HQ passkey migration applied or already present.');
  }
} finally {
