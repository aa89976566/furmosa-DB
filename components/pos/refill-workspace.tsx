...
function RefillWorkspaceInner({
  ...
  return (
    <PosShell storeName={account.storeName} account={account}>
      ...
      <div className="mx-4 mb-8 overflow-hidden rounded-[24px] border-2 border-zinc-900 bg-white shadow-[4px_4px_0_#171717] md:mx-6 md:rounded-[32px] md:shadow-[8px_8px_0_#171717]">
          ...
          {!working ? (
            <>
              <section>
                ...
                {lookupError ? (
                  <div
                    className="mt-3 space-y-2 border-l-4 border-zinc-900 pl-3"
                    role="alert"
                  >
                    <p className="break-words text-base text-zinc-800">
                      {lookupError}
                    </p>
                    <button
                      type="button"
                ...
                {totalCount === 0 ? (
                  ...
                  <ul className="mt-3 divide-y divide-neutral-200 overflow-hidden rounded-2xl border-2 border-zinc-900">
                    {visible.map((order) => {
                      ...
                      const unpaidBlock = view.unpaidBlock;
                      return (
                        <li key={order.id}>
                          <button
                            type="button"
                            ...
                            className="flex min-h-[76px] w-full items-center gap-3 bg-white px-4 py-3 text-left hover:bg-neutral-50 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-zinc-900 disabled:opacity-60"
                          >
            ...
            <RefillOrderPanel
              ...
              onCompleted={(payload) => {
                setOrders((prev) =>
                  prev.filter((order) => order.id !== selected.id),
                );
                setSelectedId(null);
                setPrefillOldSerial("");
                setSuccess(payload);
      ...
      {toast ? (
        <div
          role="status"
          className="fixed bottom-24 left-1/2 z-50 max-w-[min(24rem,calc(100%-2rem))] -translate-x-1/2 break-words rounded-full border-2 border-zinc-900 bg-white px-4 py-2 text-sm font-medium text-zinc-900 shadow-[4px_4px_0_#171717] md:bottom-8"
        >
          {toast}
        </div>
      ) : null}
