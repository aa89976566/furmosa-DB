...
export function InventorySideNav({
  ...
  return (
    <aside className="sticky top-0 hidden h-screen w-[240px] shrink-0 flex-col border-r-2 border-foreground bg-card md:flex">
      <div className="px-5 pb-4 pt-6">
        <BrandMark />
        {displayStore ? (
          <p
            className="mt-4 truncate text-sm font-medium text-zinc-900"
            title={displayStore}
          >
            {displayStore}
          </p>
      ...
      <div className="mt-auto space-y-2 border-t border-neutral-200 px-3 py-4">
        ...
        <form action={posLogoutAction}>
          <button
            type="submit"
            className={`flex min-h-11 w-full items-center rounded-xl px-3 text-sm font-medium text-zinc-700 hover:bg-neutral-100 ${NAV_LINK_FOCUS}`}
          >
            登出
          </button>
        </form>
  ...
  return (
    ...
    >
      <div
        className="grid"
        style={{
          gridTemplateColumns: `repeat(${POS_NAV.length}, minmax(0, 1fr))`,
        }}
      >
        {POS_NAV.map((tab) => {
          const Icon = ICONS[tab.id];
          ...
          return (
            ...
            >
              ...
              {isActive ? (
                <span
                  className="absolute inset-x-3 top-1 h-0.5 bg-zinc-900"
                  aria-hidden
                />
              ) : null}
              {tab.id === "stock" && itemCount > 0 ? (
                <span className="absolute right-1 top-1 flex h-5 min-w-5 items-center justify-center rounded-md bg-zinc-900 px-1 text-[10px] font-semibold text-white">
                  {itemCount}
                  <span className="sr-only"> 件待補貨</span>
                </span>
              ) : null}
