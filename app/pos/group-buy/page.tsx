...
export default async function PosGroupBuyPage(props: { searchParams?: Promise<{ q?: string | string[] }> }) {
  ...
  return <PosShell account={account}>
    <main className="space-y-5 px-4 py-6 md:px-6">
      <header><h1 className="text-2xl font-semibold">活動／團購</h1><p className="mt-2 text-sm text-muted-foreground">店家專屬進貨價。活動期間、數量與訂單請向匠寵確認。</p></header>
      ...
      {prices.length === 0 ? <p className="rounded-2xl border bg-white p-5">尚未設定專屬進貨價，請聯繫匠寵確認活動方案。</p> : visible.length === 0 ? <p>沒有符合的商品。</p> : <ul className="divide-y rounded-2xl border bg-white">
        {visible.map((price) => {
          ...
          return <li key={price.id} className="flex items-start justify-between gap-3 p-4">
            <div><h2 className="font-medium">{price.product.name}</h2><p className="mt-1 text-sm text-muted-foreground">{price.product.sku} · {price.variantKey === 'base' ? '基本規格' : tier ? tier.weightGrams != null ? `${tier.weightGrams}g` : `${tier.unitQty}${tier.unit}` : '規格待匠寵確認'}</p></div>
