...
| TBD | `cursor/line-morning-phase4b-b-optin` | （docs checkpoint 後填） | #97 branch | Phase 4B-B opt-in |
...
| RL-2026-08-08-4BB-AUTH | 2026-08-08T12:36Z | Desktop（使用者授權） | 授權開始 Phase 4B-B；先完成永久留痕（worklog）與安全 stacked branch，再做明確 opt-in UI + HQ dry-run；LINE 真送必須 0 | `PLANNED` | `cursor/line-morning-phase4b-b-optin` | （本 docs commit） | TBD | n/a docs-only | n/a |
...
- [ ] Phase 4B-B UI/flow inventory
- [ ] Opt-in 對話選項（5 選）+ 不預設 mixed
- [ ] HQ Preview dry-run UI（測試會員標記）
- [ ] ANIMAL_FACT Preview fixture 入口（必要時）
- [ ] Tests + regression + Draft PR + Preview Ready
- [ ] Ledger 更新為 IMPLEMENTED／VERIFIED

## Next action

1. UI/flow inventory（重用／缺口／wire-level flow）  
2. 實作 Phase 4B-B 最小產品範圍（見使用者授權規格）  
3. 每段更新本 Ledger

## Blockers

- 無（截至 docs checkpoint）。若 Mobile Remote 訊息未出現在本 Ledger，**不執行**。
