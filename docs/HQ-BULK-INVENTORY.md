...
- `unit=null` 的舊數字保留原值，不能當作已確認的克數。首次實體盤點後才可出貨。
...
- 狀態更新、扣帳、檢查負庫存、帳冊寫入在同一資料庫 transaction；deferred trigger 等所有明細完成後才檢查。任一商品失敗整筆回退。
...
`node scripts/ops/deploy-hq-bulk.mjs` 僅執行 `20260915130000_hq_bulk_inventory` 與六項有主鍵驗證的盤點。migration、checksum 登記、盤點同一交易提交；身份不符即全部退回。重跑不重設後續銷售餘額。不可用全量 migrate deploy 代替本次 production 步驟：production 有其他分支的歷史 schema，另有未套用的 POS migration 不在本次範圍。
