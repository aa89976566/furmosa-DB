...
| O4 | **各店商務方案實際值** | 每家店的包郵門檻、訂購級距、百分比及 `sale_time_rule`／`inbound_batch_rule`／`buyout_order_rule` 尚須由 HQ 逐店設定；不得由工程師猜測 |
...
O4 不影響基準 20%／30% 的定義，但在個別店家方案正式輸入前，不得假裝已知該店的包郵或級距條件。
