...
- 正常重試必須同時具備：persisted `refundStatus='completed'`、existing completion、相同 request fingerprint。回 `duplicate=true`、`currentPersistedStatus='completed'`、`proposedNextStatus=null` 與 stored effects，不產生新 effects。
...
- 未來 adapter 必須在同一 DB transaction 成功寫入 refund line、financial ledger、disposition／loss、inventory ledger、MerchantStock delta、idempotency result 後，最後才把 request 改成 `completed`。任一步 rollback：persisted 仍是 `approved`，且不得留下 StoredRefundCompletion。
